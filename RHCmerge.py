...
master_path  = "/Users/danielstrick/Downloads/UChicago/PH project/Advanced Hemos PAH Master Database (DS Copy).xlsx"
pi_path      = "/Users/danielstrick/Downloads/UChicago/PH project/Sequential RV-MPS in PAH (DS Copy).xlsx"
output_path  = "/Users/danielstrick/Downloads/UChicago/PH project/Sequential RV-MPS in PAH (DS Copy - Updated).xlsx"
...
RHC2_MAP = {
    ...
    77:  ("rhc2", "PAPI"),
    78:  ("rhc2", "RV-CPO (Fick)"),   # or RV-CPO (TD) — adjust if needed
    79:  ("rhc2", "RV-MPS (Fick)"),   # or RV-MPS (TD) — adjust if needed
}

# Delta columns computed from the PI sheet itself after RHC2 is filled
# col_idx: (rhc2_col_idx, rhc1_col_idx)
DELTA_MAP = {
    80: (62, 29),   # ∆MPAP   = MPAP_2   - MPAP_1
    81: (64, 31),   # ∆PA Sat = PA Sat_2 - PA Sat_1
    82: (74, 33),   # ∆SVi    = SVI_2    - SVI_1
    83: (76, 35),   # ∆Cpa    = Cpa_2    - Cpa_1
    84: (70, 38),   # ∆PVR    = TD PVR_2 - TD PVR_1
    85: (77, 40),   # ∆PAPI   = PAPI_2   - PAPI_1
    86: (78, 41),   # ∆RV-CPO = RV-CPO_2 - RV-CPO_1
    87: (79, 42),   # ∆RV-MPS = RV-MPS_2 - RV-MPS_1
}
...
for delta_idx, (rhc2_idx, rhc1_idx) in DELTA_MAP.items():
    delta_col  = pi_filtered.columns[delta_idx]
    rhc2_col   = pi_filtered.columns[rhc2_idx]
    rhc1_col   = pi_filtered.columns[rhc1_idx]

    for row_idx in range(len(pi_filtered)):
        current = pi_filtered.iloc[row_idx, delta_idx]
        if pd.notna(current):
            continue   # PI already has a value — skip
        v2 = to_num(pd.Series([pi_filtered.iloc[row_idx, rhc2_idx]])).iloc[0]
        v1 = to_num(pd.Series([pi_filtered.iloc[row_idx, rhc1_idx]])).iloc[0]
